	...
	public void doFilter(ServletRequest reques, ServletResponse respons,
		...
		try {
			...
			if (reqURI.indexOf("/home.xhtml") >= 0
					|| reqURI.indexOf("/Reports.xhtml") >= 0
					|| reqURI.indexOf("/Training.xhtml") >= 0
					|| reqURI.indexOf("/Achievements.xhtml") >= 0
					|| reqURI.indexOf("/AboutUs.xhtml") >= 0
					|| reqURI.indexOf("/Events.xhtml") >= 0

					|| (sesn != null && sesn.getAttribute("username") != null)
					|| reqURI.indexOf("/public/") >= 0
					|| reqURI.contains("javax.faces.resource")) {
				chain.doFilter(reques, respons);

				System.out.println("if loop new session");
			} else {
				// user didn't log in but asking for a page that is not allowed
				// so take user to login page
				// resp.sendRedirect(reqe.getContextPath() +
				// "/faces/home.xhtml"); // Anonymous
				// user.
				// Redirect
				// to
				// login
				// page
				System.out.println(reqe.getLocalAddr() + "/faces/home.xhtml"
						+ "--redirect to home page");

			}
		} catch (Throwable t) {
			System.out.println("do filer exception: " + t.getMessage());
		}
	} // doFilter
